...
function enqueue_slick_carousel()
{
	...
	wp_enqueue_script('slick-carousel-init', get_template_directory_uri() . '/js/slick-init.js', array('slick-carousel-js'), '', true);
}
...
?>
	<div class="filter-menu">
		<div class="filter-heading">Lọc tìm sản phẩm</div>
		<div class="filter-section">
			<div class="filter-title">Lọc theo giá</div>
			<input type="range" min="0" max="300" value="0" class="price-range" id="priceRange">
			<button class="filter-btn" onclick="filterProducts()">Lọc</button>
			<div class="price-label">Giá <span id="priceValue">0</span> — 300,000</div>
		</div>
		<div class="filter-section">
			<div class="filter-title">Theo màu sắc</div>
			<input type="checkbox" id="colorFilter">
		</div>
		<div class="filter-section">
			<div class="filter-title">Theo chất liệu</div>
			<label><input type="checkbox" value="SUS201"> SUS201</label>
			<label><input type="checkbox" value="SUS304"> SUS304</label>
		</div>
		<div class="filter-section">
			<div class="filter-title">Theo hãng</div>
			<label><input type="checkbox" value="Nam Hải"> Nam Hải</label>
		</div>
	</div>
	<script>
...
add_shortcode('custom_filter_menu', 'custom_filter_menu');
...
function enqueue_custom_scripts()
{
	...
	wp_enqueue_style('slick-carousel-css', get_template_directory_uri() . '/slick/slick.css', array(), '1.8.1');
	wp_enqueue_style('slick-carousel-theme-css', get_template_directory_uri() . '/slick/slick-theme.css', array(), '1.8.1');

	// Enqueue Slick Carousel JS
	wp_enqueue_script('slick-carousel-js', get_template_directory_uri() . '/slick/slick.min.js', array('jquery'), '1.8.1', true);

	// Enqueue custom JS
	wp_enqueue_script('custom-js', get_template_directory_uri() . '/js/custom.js', array('jquery', 'slick-carousel-js'), null, true);
}
add_action('wp_enqueue_scripts', 'enqueue_custom_scripts');

function custom_enqueue_scripts()
{
	wp_enqueue_script('jquery'); // Đảm bảo jQuery được tải lên
?>
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$('.filter-button').on('click', function() {
				fetch_products(1); // Bắt đầu từ trang 1
				...
				function fetch_products(page) {
					$.ajax({
						...
						success: function(response) {
							...
							$('.all-products').html(result.products).show(); // Hiển thị phần hiển thị sản phẩm
							$('.all-categories').hide(); // Ẩn phần hiển thị danh mục con
							$('.products').hide(); // Ẩn phần hiển thị danh mục con
							$('.pagination').html(result.pagination); // Cập nhật phân trang
						},
